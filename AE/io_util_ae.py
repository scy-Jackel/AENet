...
import threading
import math
# tensorflow default image data_format is channel last.

# VERSION 1: VMAX: 3072, VMIN: -1024
# VERSION 2: VMAX: 240, VMIN:-160

class DCMDataLoader(object):
    def __init__(self, dcm_path, LDCT_image_path, NDCT_image_path, \
                 image_size = 512, patch_size = 64,  depth = 1, \
                 image_max = 240, image_min = -160, batch_size = 1, \
    ...
    def __call__(self, patent_no_list):
        ...
        for patent_no in patent_no_list:
            ...
            if len(P_LDCT_path)*len(p_NDCT_path)==0:
                print('error, len of data dir is 0!!')
                print('P_LDCT_path:',os.path.join(self.dcm_path, patent_no, self.LDCT_image_path, '*.' + self.extension))
                print('p_NDCT_path:',os.path.join(self.dcm_path, patent_no, self.NDCT_image_path, '*.' + self.extension))
            ...
            #CT slice name
            self.input_image_name.extend(LDCT_slice_nm)
            self.input_image_name.extend(NDCT_slice_nm)

            #normalization  
            p_all.append(self.normalize(org_LDCT_images, self.image_max , self.image_min))
            p_all.append(self.normalize(org_NDCT_images, self.image_max , self.image_min))
            
        self.input_images = np.concatenate(tuple(p_all), axis=0)
        
        #image index
        self.input_index = list(range(len(self.input_images)))
    ...
    def normalize(self, img, max_ = 240, min_=-160):
        ...
        def enqueue(coord):
            ...
            while not coord.should_stop():
                ...
                for i in range(enqueue_size):
                    ...
                    random_index = np.random.choice(self.input_index)
                        
                    pat_input, pat_output = \
                        self.get_randam_patches(self.input_images[random_index],
                            self.input_images[random_index], image_size)
                    input_imgs.append(np.expand_dims(pat_input, axis=-1))
                    output_imgs.append(np.expand_dims(pat_output, axis=-1))
                sess.run(enqueue_op, feed_dict={queue_input: np.array(input_imgs),queue_output: np.array(output_imgs)})
                self.step += 1
